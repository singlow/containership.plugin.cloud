...
function login(method) {
    ...
    async.waterfall([
        (cb) => {
            return prmpt.get([{
                name: 'username',
                description: chalk.white(method === 'github' ? 'Username' : 'Email'),
                required: true
            }, {
                name: 'password',
        ...
        (credentials, needsOtp, cb) => {
            ...
            if (needsOtp) {
                return prmpt.get([{
                    name: 'otp',
                    description: chalk.white('Authentication Code'),
                    required: true
                }], (err, res) => {
                    if (err) {
                        return cb(err);
                    ...
                    return github.authorize(res.otp, cb);
                });
            } else {
                return github.authorize(null, cb);
            }
        }
    ], (err, auth) => {
        if (err) {
            let err_msg = err.message || 'There was an error logging in!';

            if (err.code === 401) {
                if (err.message.indexOf('Must specify two-factor authentication OTP code.') > 0) {
                    err_msg = 'The OTP code provided was not valid!';
                } else {
        ...
        return request(request_options, (err, response) => {
            ...
            conf.plugins.cloud.config.token = `${response.body.token}`;
            ...
            conf.metadata.request.headers.authorization = `Bearer ${response.body.token}`;
    ...
    commands: [
        ...
        },
        {
            ...
            callback: (argv) => {
                ...
                if (!argv.name && !argv.phone && !argv.email) {
                    return console.error('You must provide either name, email, or phone as a flag to edit');
                }
...
function getOrganization(id, token, cb) {
    if (!token) {
        return cb('You must be logged in to list organizations!');
    }
    ...
    return request(request_options, function(err, response) {
        if(err || response.statusCode != 200) {
            return cb('You do not have permissions to use this organization!');
        }
...
function getCluster(org_id, cluster_id, token, cb) {
    if (!token) {
        return cb('You must be logged in to list clusters!');
    }
    ...
    return request(request_options, function(err, response) {
        if(err || response.statusCode !== 200) {
            return cb('You do not have permissions to use this cluster!');
        }
    ...
    commands: [
        {
            ...
            callback: () => {
                ...
                if (!org_id) {
                    return console.error('You must have an active_organization to list clusters. See `cscli cloud org use <org_id>`.');
                }
        ...
        },
        {
            ...
            callback: (argv) => {
                ...
                if (!org_id) {
                    return console.error('You must have an active_organization to editr a cluster. See `cscli cloud organization use <org_id>`.');
                }
        ...
        },
        {
            ...
            callback: (args) => {
                ...
                if (!org_id) {
                    return console.error('You must have an active_organization to use a cluster. See `cscli cloud organization use <org_id>`.');
                }

                return getCluster(org_id, args.cluster_id, token, (err, cluster) => {
                    if (err) {
                        return console.error(err);
                    }
        ...
        },
        {
            ...
            callback: (args) => {
                ...
                if (!org_id) {
                    return console.error('You must have an active_organization to list clusters. See `cscli cloud organization use <org_id>`.');
                }
