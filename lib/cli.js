...
module.exports = {
        ...
        callback: function() {
            ...
            request(request_options, function(err, response) {
                ...
                async.forEach(response.body.organizations || [], function(organization, fn) {
                    ...
                    logger.info(sprintf('%-50s %-50s %-50s',
                        'ID',
                        'ORGANIZATION',
                        'OWNER'
                    ));
        ...
        callback: function(options) {
            ...
            request(request_options, function(err, response) {
                ...
                if(_.find(response.body.organizations, org => org.id === options.organization)) {
                    ...
                    config.set(config.config);
                    logger.info(['Successfully switched to', options.organization, 'organization!'].join(' '));
                } else {
                    ...
                    if(_.has(clusters, options['cluster-id']) && clusters[options['cluster-id']].provider === 'vag') {
                        providers.vagrant.validate(function(err) {
                            ...
                            providers.vagrant.destroy(function(err) {
                                ...
                                logger.info(['Successfully destroyed cluster:', options['cluster-id']].join(' '));
                            });
                        });
                    } else {
    ...
    'use-cluster': {
        ...
        callback: function(options) {
            ...
            config.set(config.config);
            logger.info(['Successfully updated config to use cluster', options['cluster-id']].join(' '));
        }
    },
        ...
        callback: function() {
            ...
            providers.vagrant.get_clusters(function(err, clusters) {
                ...
                request(request_options, function(err, response) {
                    ...
                    logger.info(sprintf('%-40s %-40s %-25s %-15s %-10s %-10s',
                        'ID',
                        'NAME',
                        'PROVIDER',
                        'APPLICATIONS',
        ...
        callback: function() {
            ...
            async.waterfall([
                ...
                if(err) {
                    ...
                    logger.info(['Starting build with ID:', build_id].join(' '));
                    let options = {
