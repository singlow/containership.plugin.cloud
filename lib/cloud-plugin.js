...
const { ContainershipPlugin, ApiBuilder } = require('@jeremykross/containership.plugin');
...
class ContainershipCloudPlugin extends ContainershipPlugin {
    ...
    apiRequest(verb, endpoint, data, cb) {
        ...
        if(api_key && organization) {
            const options = {
                baseUrl: "http://jeremykross.com:8081", // constants.environment.CLOUD_API_BASE_URL,
                url: endpoint,
                method: verb,
                timeout: 15000,
                headers: {
