    ...
    set_rules() {
            ...
            this.rule_queue = async.queue((task, callback) => {
                ...
                if(task.chain === this.options.chain.input) {
                    return async.parallel({
                        ...
                        if(err) {
                            console.log('Unable to fetch cloud firewall rules!');
        ...
        _.forEach(this.options.chain, (containership_chain, chain) => {
            this.rule_queue.push({
                ...
                if(err) {
                    console.log(`Unable to apply new firewall rules to chain ${chain}!`);
                    console.log(err.message);
                } else{
                    console.log(`Sucessfully applied new firewall rules to chain ${chain}!`);
                }
            });
        });
    }
