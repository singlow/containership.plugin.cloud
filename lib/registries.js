...
class Registries {
    constructor(config, host) {
        ...
        if(orchestrator === 'kubernetes') {
            ...
            logger.error(`Invalid host orchestrator: ${orchestrator}. Refusing to operate on registries`);
        }
    }
    ...
    get_registry_configs(callback) {
        this.host.getApi().getDistributedKey(constants.myriad.REGISTRIES, (err, registries) => {
            if(err || !registries) {
                logger.error('Error reading registries from distributed state');
                return callback(err);
            } else {
                registries.push({
    ...
    write_registries_to_file() {
        logger.info('Writing registries to file');

        this.get_registry_configs((err, registry_configs) => {
            if(err) {
                logger.error(`Could not write registries to file: ${err.message}`);
            } else {
                const docker_config = {
                    auths: {},
                    HttpHeaders: {
                ...
                fs.mkdir(DOCKER_CONFIG_DIR, (err) => {
                    if(err && err.code !== 'EEXIST') {
                        logger.error(`Failed to create docker config directory (${DOCKER_CONFIG_DIR}): ${err.message}`);
                    } else {
                        // overwrite existing docker auth config file
                        fs.writeFile(DOCKER_AUTH_CONFIG_FILE, JSON.stringify(docker_config, null, 2), (err) => {
                            if(err) {
                                logger.error(`Error writing docker auth config file: ${err.message}`);
                            }
                        });
                    }
                });
    ...
    inject_registries(options, callback) {
        logger.info('Injecting auth middleware');
        ...
        this.get_registry_configs((err, registry_configs) => {
            if(err) {
                logger.error(`Could not inject registries into middleware ${err.message}`);
                return callback();
            } else {
